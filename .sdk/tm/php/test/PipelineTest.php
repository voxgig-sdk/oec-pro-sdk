...
{
    private static function utility(): OecProUtility
    {
        return new OecProUtility();
    }
    ...
    private static function ctx(array $over = []): OecProContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OecProContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OecProOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OecProError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OecProOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OecProMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OecProOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OecProMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OecProOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OecProMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OecProSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OecProMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OecProResponse([]),
            'result' => new OecProResult([]),
        ]);
        [, $err] = OecProMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OecProSpec(['step' => 's']),
            'response' => new OecProResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OecProResult([]),
        ]);
        [, $err] = OecProMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OecProSpec(['step' => 's']),
            'response' => new OecProResponse(self::resp(200, ['v' => 1])),
            'result' => new OecProResult([]),
        ]);
        [, $err] = OecProMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OecProSpec(['step' => 's']),
            'response' => new OecProResponse(self::resp(200, ['v' => 2])),
            'result' => new OecProResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OecProMakeResponse::call($ctx);
    ...
    {
        $preset = new OecProResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OecProSpec([]),
            'response' => new OecProResponse([]),
            'result' => new OecProResult([]),
        ...
        [$got, $err] = OecProMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OecProResult([])]);
        [, $err] = OecProMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OecProSpec([]), 'result' => null]);
        [, $err] = OecProMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OecProSpec(['step' => 's']),
            'result' => new OecProResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OecProOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OecProMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OecProSpec(['step' => 's']),
            'result' => new OecProResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OecProOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OecProMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OecProResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OecProSpec([]),
            'result' => new OecProResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OecProMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OecProMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OecProError('boom', 'boom');
        $utility->fetcher = function (OecProContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OecProSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OecProMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OecProContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OecProMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OecProResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OecProContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OecProSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OecProMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OecProResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OecProSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OecProMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OecProMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OecProMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OecProResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OecProDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OecProResult(['ok' => false])]);
        $this->expectException(OecProError::class);
        OecProDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OecProResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OecProMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OecProBaseFeature();
        $b = new OecProBaseFeature();
        OecProFeatureAdd::call($ctx, $a);
        OecProFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OecProBaseFeature
    {
        $f = new OecProBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OecProFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OecProContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OecProSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OecProPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OecProResponse(['headers' => null]),
            'result' => new OecProResult([]),
        ]);
        OecProResultHeaders::call($ctx);
