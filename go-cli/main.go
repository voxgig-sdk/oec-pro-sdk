// Command OecPro-cli is a CLI + REPL for the OecPro SDK,
...
const prompt = "oec-pro"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "country product trade"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OEC_PRO_APIKEY (api key), OEC_PRO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OEC_PRO_APIKEY carries the API key and
	// OEC_PRO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OEC_PRO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OEC_PRO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOecProSDK(opts)
