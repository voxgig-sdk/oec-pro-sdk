...
MCP server exposing the OecPro SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `oec-pro_list` | `entity`, optional `query` map | First page of records as JSON |
| `oec-pro_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `oec-pro_list` and
`oec-pro_load` tools become available in new sessions.
