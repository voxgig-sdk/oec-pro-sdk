...
[MCP](https://modelcontextprotocol.io) server exposing the OecPro SDK as
two agent tools — `oec-pro_list` and `oec-pro_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/oec-pro-mcp)
make build

# 2. Provide credentials via the environment
export OEC_PRO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user oec-pro \
  -- /absolute/path/to/oec-pro-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./oec-pro-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// oec-pro_list: first page of records
{ "entity": "country" }
{ "entity": "country", "query": { } }

// oec-pro_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/oec-pro-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user oec-pro \
     -- "$PWD"/dist/*/oec-pro-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `oec-pro_list` and `oec-pro_load` tools now appear
   in new sessions. Ask the agent to *"list country using oec-pro"*
   and it calls `oec-pro_list` with `{"entity":"country"}`.
...
```sh
export OEC_PRO_APIKEY=sk_live_xxx            # API key
export OEC_PRO_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `oec-pro_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `oec-pro_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OEC_PRO_APIKEY` | API key sent with every request. |
| `OEC_PRO_BASE` | Optional override of the API base URL. |
...
```sh
./oec-pro-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"oec-pro_load","arguments":{"entity":"country","query":{"id":1}}}}'
```
