...
import (
	...
	omni "github.com/voxgig-sdk/oec-pro-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OecProError); is {
				out := map[string]any{
					"name":    "OecProError",
...
func omniCtx(arg any, client *sdk.OecProSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OecProSDK); is {
			client = live
