...
type productDirectSetupResult struct {
...
func productDirectSetup(mockres any) *productDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"OEC_PRO_TEST_PRODUCT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["OEC_PRO_TEST_PRODUCT_ENTID"]; ok {
		...
		return &productDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &productDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
