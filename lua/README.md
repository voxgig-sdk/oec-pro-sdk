...
The Lua SDK for the OecPro API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Country()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local trades, err = client:Trade():list()
```
...
```
lua/
├── oec-pro_sdk.lua    -- Main SDK module
...
The main module (`oec-pro_sdk`) exports the SDK constructor
