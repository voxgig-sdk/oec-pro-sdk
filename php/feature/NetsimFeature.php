...
class OecProNetsimFeature extends OecProBaseFeature
{
    ...
    private function _simulate(OecProContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(OecProContext $ctx, array $applied): void
    {
        $client = $this->client;
