...
class OecProSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = OecProHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = OecProHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!OecProFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, OecProFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return OecProUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = OecProHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = OecProHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = OecProHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new OecProError($op . "_allow",
                "OecProSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = OecProHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new OecProError("graphql_error",
                "OecProSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
