...
class FtCtrl extends OecProControl
{
...
class FtClient
{
    ...
    private OecProUtility $utility;

    public function __construct(OecProUtility $utility, array $options)
    {
    ...
    public function get_utility(): OecProUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OecProContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OecProContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OecProConfig::make_feature($name);
    ...
    private static function buildUrl(OecProSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OecProError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OecProResponse($fetched);
        }
    ...
    private function fail(OecProContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OecProContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
