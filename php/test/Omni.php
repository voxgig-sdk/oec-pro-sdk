...
//    only - a typed OecProContext there would make every ctx
...
final class OecProOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new OecProContext($clean, null);
    ...
    public static function err_from_map($m): ?OecProError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new OecProError(is_string($code) ? $code : '', $msg);
    }
