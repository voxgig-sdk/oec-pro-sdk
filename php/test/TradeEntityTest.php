...
class TradeEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = OecProSDK::test($seed, null);
        $seen = iterator_to_array($base->Trade(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Trade(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OEC_PRO_TEST_TRADE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function trade_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("OEC_PRO_TEST_TRADE_ENTID");
