...
class TestTradeDirect:
    ...
    def test_should_direct_list_trade(self):
        setup = _trade_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-trade", "live" if setup["live"] else "unit")
        if _skip:
