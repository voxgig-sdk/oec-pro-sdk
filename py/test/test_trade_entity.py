...
class TestTradeEntity:
    ...
    def test_should_stream(self):
        ...
        base = OecProSDK.test(seed, None)
        seen = list(base.Trade(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Trade(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set OEC_PRO_TEST_TRADE_ENTID JSON to run live")
        client = setup["client"]
...
def _trade_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("OEC_PRO_TEST_TRADE_ENTID"))
    ...
    if env.get("OEC_PRO_TEST_LIVE") == "TRUE":
        ...
        client = OecProSDK(helpers.to_map(merged_opts))

    _live = env.get("OEC_PRO_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("OEC_PRO_TEST_EXPLAIN") == "TRUE",
        "live": _live,
