...
The Ruby SDK for the OecPro API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Country` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = OecProSDK.test(testopts, sdkopts)
```
...
raise a `OecProError` (a `StandardError` subclass), so wrap
...
The main module (`OecPro_sdk`) exports the SDK class
