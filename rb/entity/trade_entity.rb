...
class TradeEntity
  ...
  def data_set(args)
    if args
      @_data = OecProHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = OecProHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [TradeListMatch, Hash, nil] match filter (any subset of
  #   Trade fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Trade>, Array] the matching Trade items; raises OecProError on failure
