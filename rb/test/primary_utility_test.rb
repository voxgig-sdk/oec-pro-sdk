...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = OecProOmni.make_runner(TEST_JSON_FILE, OecProSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : OecProOmni.errify(err)
    end
