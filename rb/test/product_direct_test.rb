...
class ProductDirectTest < Minitest::Test
  def test_direct_list_product
    setup = product_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-product", setup[:live] ? "live" : "unit")
    if _should_skip
...
def product_direct_setup(mockres)
