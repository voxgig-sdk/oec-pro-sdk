...
class TradeEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = OecProSDK.test(seed, nil)
    seen = base.Trade(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Trade(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OEC_PRO_TEST_TRADE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def trade_basic_setup(extra)
  ...
  entid_env_raw = ENV["OEC_PRO_TEST_TRADE_ENTID"]
  ...
  if env["OEC_PRO_TEST_LIVE"] == "TRUE"
    ...
    client = OecProSDK.new(Helpers.to_map(merged_opts))
  end
