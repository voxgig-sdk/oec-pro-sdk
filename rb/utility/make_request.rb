# OecPro SDK utility: make_request
...
module OecProUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = OecProResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OecProResponse.new(fetched)
    else
