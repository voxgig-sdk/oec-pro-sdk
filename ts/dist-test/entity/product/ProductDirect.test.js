...
(0, node_test_1.describe)('ProductDirect', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when OEC_PRO_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('OEC_PRO_TEST_LIVE'));
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.OecProSDK({
    ...
    (0, node_test_1.test)('direct-list-product', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-product', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.OEC_PRO_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.OecProSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.OEC_PRO_APIKEY,
        }));
        let idmap = env['OEC_PRO_TEST_PRODUCT_ENTID'];
    ...
    const client = new __1.OecProSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ProductDirect.test.js.map