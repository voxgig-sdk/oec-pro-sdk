...
//     (\`new OecProSDK(...)\` / \`OecProSDK.test(...)\` become
//     \`OecProSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"country":{"test01":{"id":"test01"}},"product":{"test01":{"id":"test01"}},"trade":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OecProSDK(...)\` and any
// \`OecProSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OecProSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OecProSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
